...
togglePassword.addEventListener('click', function (e) {
  ...
  const type = password.getAttribute('type') === 'text' ? 'text' : 'password';
...
function addStu() {
  ...
  var pass2 = $("#pass2").val();
  var dob = $('#dob').val();
// {/* <button type="submit" class="btn btn-danger text-white font-weight-bolder float-right me-3" name="cart">Add to Cart</button>; */}

  //checking  all feilds on form submission 
  if( stuname.trim(), stupass.trim(), stuemail.trim() , pass2.trim(), dob.trim() == "") {
    $("#statusMsg1").html(
    ...
    $("#dob").focus();
    $("#statusMsg5").html(
      '<small style="color:red;">  Please Enter Date ! </small>'
    );
    return false;
  }
  ...
  if (stuname.trim() == "") {
    ...
    $("#statusMsg5").html(
      '<small style="color:red;"> Please Enter Date of Birth </small>'
    );
    $("#stupass").focus();
    return false;
